...
int    picoshell(char **cmds[])
{
   ...
   // int status;  // for wati();
	...
	// Esperar a todos los hijos y capturar sus estados
	while(wait(&status) > 0)
	{ 
		// Si el hijo terminó pero no de forma exitosa (status != 0)
		if (WIFEXITED(status) && WEXITSTATUS(status) != 0)
			exit_code = 1;
		// Si el hijo terminó por una señal (opcional, pero buena práctica)
